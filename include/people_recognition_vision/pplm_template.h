...
class PPLMatcherTemplate : public vision_utils::NanoSkill {
public:
  ...
  /*! to be implemented by children classes.
     ...
     ie new_ppl.people.size() * tracks.people.size().
  ...
  virtual bool match(const PPL & new_ppl, const PPL & tracks,
                     ...
                     std::vector<unsigned int> & tracks_added_indices) = 0;
...
private:
  ...
  virtual bool match_cb(people_recognition_vision::MatchPPL::Request& request,
    ...
    bool ok = match(request.new_ppl, request.tracks,
                    response.costs,
                    response.new_ppl_added_tagnames,
                    response.new_ppl_added_tags,
                    response.new_ppl_added_indices,
                    response.tracks_added_tagnames,
                    response.tracks_added_tags,
                    response.tracks_added_indices);
